...
├── js/
│   ├── config.js         # URL + anon key do Supabase (EDITE aqui)
...
2. Em **SQL Editor**, cole e rode todo o `supabase/schema.sql` (cria `perfil`, `sessoes`, `eventos` + RLS).
3. Em **Authentication → Providers**: mantenha **Email** habilitado. Para permitir “entrar como convidado”, habilite **Anonymous sign-ins**. (Opcional: desligar confirmação de e-mail em ambiente de teste, em Authentication → Sign In / Providers → Email.)
4. Em **Project Settings → API**, copie **Project URL** e **anon public**.
5. Edite `js/config.js`:
   ```js
   ...
   ```
6. Em **Authentication → URL Configuration**, adicione a URL do seu site (ex.: a do GitHub Pages/Vercel) em *Site URL* e *Redirect URLs*.

Enquanto `config.js` estiver com os placeholders, o jogo roda em **modo local** (sem login/persistência).
